...
describe('bus-route storage', () => {
  ...
  it('treats malformed JSON as absent', () => {
    localStorage.setItem('riddler:bus-route:best:easy', '{not json');
    expect(getBest('easy')).toBeNull();
  });

  it('treats malformed shape as absent', () => {
    localStorage.setItem('riddler:bus-route:best:easy', JSON.stringify({ foo: 'bar' }));
    expect(getBest('easy')).toBeNull();
  });
