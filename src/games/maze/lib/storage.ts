...
function storageKey(difficultyId: DifficultyId): string {
  return `riddler:maze:best:${difficultyId}`;
}
 ...
 * HIGHER (best = max; higher-is-better for the maze). Always increments the
 * play count. Safe no-op (with best-effort return value) if localStorage is
