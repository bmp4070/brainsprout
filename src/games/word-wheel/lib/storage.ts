...
function storageKey(difficultyId: DifficultyId): string {
  return `riddler:word-wheel:best:${difficultyId}`;
}
...
/**
 * Records a completed round's score, updating the best score if higher.
 * Always increments play count. Safe no-op if localStorage is unavailable.
 */
