...
describe('spot-difference storage', () => {
  ...
  it('uses the spot-difference storage key namespace', () => {
    recordResult('easy', 555);
    expect(localStorage.getItem('riddler:spot-difference:best:easy')).not.toBeNull();
  });

  it('treats malformed JSON as absent', () => {
    localStorage.setItem('riddler:spot-difference:best:easy', '{not json');
    expect(getBest('easy')).toBeNull();
  });

  it('treats malformed shape as absent', () => {
    localStorage.setItem('riddler:spot-difference:best:easy', JSON.stringify({ foo: 'bar' }));
    expect(getBest('easy')).toBeNull();
  });
