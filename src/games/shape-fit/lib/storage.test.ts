...
describe('shape-fit storage', () => {
  ...
  it('uses the shape-fit storage key', () => {
    recordResult('easy', 650);
    expect(localStorage.getItem('riddler:shape-fit:best:easy')).toBe(
      JSON.stringify({ bestScore: 650, plays: 1 }),
    );
  });

  it('treats malformed JSON as absent', () => {
    localStorage.setItem('riddler:shape-fit:best:easy', '{not json');
    expect(getBest('easy')).toBeNull();
  });

  it('treats malformed shape as absent', () => {
    localStorage.setItem('riddler:shape-fit:best:easy', JSON.stringify({ foo: 'bar' }));
    expect(getBest('easy')).toBeNull();
  });
