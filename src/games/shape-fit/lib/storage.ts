...
function storageKey(difficultyId: DifficultyId): string {
  return `riddler:shape-fit:best:${difficultyId}`;
}
...
/**
 * Records a completed play's score, updating the best only if `score` is HIGHER
 * (best = max). Always increments the play count. Safe no-op (with best-effort
 * return value) if localStorage is unavailable or corrupt.
 */
